...
/** Atomic OrbitX Client profile installer. */
public final class CsClientInstaller {
    ...
    private static void run(Context ctx,CsClientCatalog.VersionEntry blaze,List<CsClientCatalog.ProjectEntry> mods,CsClientCatalog.ProjectEntry pack,Callback cb){
        ...
        try{
            ...
            String loader=null;for(FabricVersion candidate:loaders)if(candidate.stable&&versionAtLeast(candidate.version,blaze.minimumLoader)){loader=candidate.version;break;}if(loader==null)throw new Exception("OrbitX Client requires Fabric Loader "+blaze.minimumLoader+" or newer");
            ...
            status(cb,"Installing bundled OrbitX Client…",48);CsClientManagedFiles.installCore(ctx,blaze,tmp);
            ...
            status(cb,"Finalizing OrbitX Client profile…",92);dest.getParentFile().mkdirs();if(!tmp.renameTo(dest))throw new Exception("Could not finalize instance directory");
            LauncherProfiles.load();String key=LauncherProfiles.getFreeProfileKey();MinecraftProfile p=MinecraftProfile.createTemplate();p.name="OrbitX Client • "+blaze.minecraft;p.type="custom";p.lastVersionId=versionId;p.gameDir="./custom_instances/"+token;p.icon=encodeLogo(ctx);p.background=net.kdt.pojavlaunch.profiles.ProfileGifSupport.DEFAULT_PROFILE_BG_URL;
            ...
            done(cb,true,"OrbitX Client • "+blaze.minecraft+" created");
        }catch(Throwable e){try{org.apache.commons.io.FileUtils.deleteDirectory(tmp);}catch(Exception ignored){}try{org.apache.commons.io.FileUtils.deleteDirectory(dest);}catch(Exception ignored){}done(cb,false,e.getMessage()==null?"OrbitX Client installation failed":e.getMessage());}
    }
