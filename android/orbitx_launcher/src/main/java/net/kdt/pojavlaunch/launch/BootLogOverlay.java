...
public class BootLogOverlay extends FrameLayout {
    ...
    /**
     * OrbitX — dim or hide the whole "starting overlay".
     *
     * @param opacity 0f (invisible — nothing is drawn) … 1f (fully opaque)
     */
    public void setColumnOpacity(float opacity) {
        if (mColumn != null) mColumn.setAlpha(Math.max(0f, Math.min(1f, opacity)));
    }
    ...
    public void start() {
        ...
        appendStyled(colour("› orbitx · boot log", 0xFF9AA0AC));
    ...
    private void flush() {
        ...
        mCounter.setTextColor(mErrors > 0 ? 0xFFFF6B6B : mWarnings > 0 ? 0xFFFFA726 : 0xFF6B7280);
    }
    ...
    private CharSequence style(String raw) {
        ...
        if (up.contains("FATAL") || up.contains("ERROR") || up.contains("EXCEPTION") || up.contains("CRASH")) {
            color = 0xFFFF6B6B; mErrors++;
        } else if (up.contains("WARN")) {
            color = 0xFFFFA726; mWarnings++;
